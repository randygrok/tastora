...
func BuildInternalPeerAddressList(ctx context.Context, nodes []types.Node) (string, error) {
...
// It returns an error if any node fails to provide a peer address.
func BuildInternalRPCAddressList(ctx context.Context, nodes []types.Node) (string, error) {
	addrs := make([]string, 0, len(nodes))
	for _, node := range nodes {
